...
public class RunTabManualTriggers extends VLayout {
			...
			public void onClick(ClickEvent event) {
				for (ListGridRecord userRec : runTab.getUsersCanvas().getSelectedRecords()) {
					...
					for (ListGridRecord actionRec : actionGrid.getSelectedRecords()) {
						...
						ActionClient.getInstance().notify(runTab.getRunId(), userRec.getAttribute("email"), actionRec.getAttribute("itemId"),
								new JsonCallback() {
	...
	public void refreshSources() {
		if (actionGrid != null) {
			GameClient.getInstance().getGameConfig(runTab.getGameId(), new JsonCallback() {

				public void onJsonReceived(JSONValue jsonValueOld) {
					JSONObject jsonValue = jsonValueOld.isObject();
					TriggerDataSource.getInstance().processGameConfig(runTab.getGameId(), jsonValue);
