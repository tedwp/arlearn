...
public class FilterForm extends DynamicForm {
	...
	private SelectItem addSelectTeamField() {
		final SelectItem select = new SelectItem(TeamModel.TEAMID_FIELD, "team -i18"); //TODO add internationalisation constant  
	...
	private SelectItem addSelectItemField() {
		final SelectItem select = new SelectItem(GeneralItemModel.GENERALITEMID_FIELD, "item -i18"); //TODO add internationalisation constant  
		select.setMultiple(true);  
		select.setWidth(150);
	...
	private SelectItem addSelectRolField() {
		final SelectItem select = new SelectItem(RunRoleModel.ROLE_FIELD, "role -i18"); //TODO add internationalisation constant  
        select.setMultiple(true);  
        select.setWidth(150);
	...
	private SelectItem addSelectUserField() {
		final SelectItem select = new SelectItem("userEmail", "user -i18"); //TODO add internationalisation constant  
        select.setMultiple(true);  
        select.setWidth(150);
        select.setOptionDataSource(UserDataSource.getInstance());
        select.setAutoFetchData(true);
        select.setValueField(UserModel.EMAIL_FIELD);
