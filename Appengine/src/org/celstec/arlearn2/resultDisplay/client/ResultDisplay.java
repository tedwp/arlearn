...
public class ResultDisplay implements EntryPoint {
	...
	public void onModuleLoad() {
		if (!Authentication.getInstance().isAuthenticated()) {
			Authentication.getInstance().userCredentialsReceived("arlearn1", "");
		} else {
...
//		main.addMembers(filterForm, hLayout, tileGrid);		
	...
	private DynamicForm setSearchTileGridForm(final TileGrid tileGrid) {
        ...
        final SelectItem selectMultiUser = new SelectItem("userEmail");  
...
//	}
	...
	public void showResponses() {
		...
		if (!Authentication.getInstance().isAuthenticated()) {
			Authentication.getInstance().userCredentialsReceived("arlearn1", "arl3arn123");
		} else {
			System.out.println(Authentication.getInstance().getAuthenticationToken());
		}
